...
cbor_item_t *ctf_tbl_pop(struct c_trace_fwd_state *state)
{
	cbor_item_t *item;
	size_t pos = state->item_tbl_pos;

	if (pos < 0)
		return NULL;
	item = state->item_tbl[pos];
	state->item_tbl[pos] = NULL;
	state->item_tbl_pos--;
	return item;
}

int ctf_tbl_push(struct c_trace_fwd_state *state, cbor_item_t *item)
{
	size_t pos = state->item_tbl_pos;

	if (pos + 1 < state->item_tbl_sz && ctf_tbl_expand(state))
		return RETVAL_FAILURE;
	state->item_tbl[pos] = item;
