...
continue_for_loop:
		if (!(result = ctf_proto_stk_decode(fd))) {
			ctf_msg(ctf_alert, state, "decode failed, "
		...
		case CBOR_ERR_MALFORMATED:
			ctf_msg(ctf_warning, state,
					"got CBOR_ERR_MALFORMATED, "
					"but continuing until EOF "
					"anyway\n");
			cpsdr_free(result);
			goto continue_for_loop;
		case CBOR_ERR_MEMERROR:
			ctf_msg(ctf_warning, state, "got CBOR_ERR_MEMERR, "
					"but continuing until EOF "
					"anyway\n");
			cpsdr_free(result);
			goto continue_for_loop;
...
setup_unix_sock(int *fd, struct sockaddr *sockaddr)
{
	*fd = socket(AF_UNIX, SOCK_STREAM, 0);
	if (*fd < 0) {
		ctf_msg(ctf_emergency, state,
				"socket creation for Unix sock failed\n");
		return false;
	}
...
setup_state(struct ctf_state **state, struct ctf_conf *conf)
{
	...
	if (!(*state = g_rc_box_new0(struct ctf_state))) {
		ctf_msg(ctf_alert, state, "g_rc_box_new0() failed\n");
	...
	if (page_size < 0) {
		ctf_msg(ctf_emergency, state, "page size detection failed\n");
